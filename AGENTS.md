...
- **WebUI 记忆管理页**（`lib/admin.js` + `client/client.js`，0.1.19）：Host 用 `webServer` 注册 `/api/persona-memory/*` 路由（status/checkModel/configSave/update/delete/standingAdd/standingUpdate/standingRemove/rebuildVector/projectUpdate/projectDelete/backup/restoreLatest，**loopback-only 防护**，同 dsh-ssh 围栏）；Client 是**预构建 bundle**（`window.__ModuleLoader__.load` 格式，经 `dsh.client` + `exports["./client"]` 声明加载，**非动态插件无 harness.handle**，用同源 fetch 调路由）；页面注册到 `settings.section` slot（id=persona-memory，order=25，label=记忆管理）；卡片=常驻指令/记忆文件/项目记忆/检索索引/本地备份/插件配置；**检索索引卡=纯状态**（FTS5/向量大小、重建按钮，**不含任何配置项**——向量配置统一在插件配置卡，与 v27 定稿一致）；**插件配置卡**按 `CFG_SCHEMA`（40 项，分组：基础/学习/合并/检索/向量/项目/常驻/失败/管理/备份）schema 驱动渲染（bool→checkbox、number→数字框、select→按钮组、string→输入框），**embeddingModel 特殊渲染**：输入框+缓存检测+已下载模型按钮组（点击选用），保存写回当前 profile 的 `cordis.patch.yml` 中 persona-memory 段（`adminProfile` 配置项指定 profile 名，默认 web）；**本地备份卡**：备份目录 `$DSH_HOME/memory-backup/latest/`（与 Pi 完全隔离、只保留最新快照，8 文件=MEMORY/USER/failures/STANDING + projects-*.md），手动「立即备份/从最新备份恢复」；**自动保护**（`lib/admin.js` `startAutoProtect`，index.js 启动，ctx.timer.interval 每分钟 tick）：按 `autoBackupMin`（默认 60，0=关）间隔覆盖备份 + Pi 丢失检测（`autoSwitchOnPiLoss`，Pi 曾见 MEMORY.md 后消失 → patch 写 `dir: $DSH_HOME/memory` + restoreFromLatest）；**Client 代码修改后必须重新 `pnpm add` + 重启 web 才生效**（预构建产物按包分发）；client 只依赖 react + 浏览器 fetch，**不 import 任何 @deepseek-ai client 包运行时值**（peerDependencies 已声明 react、dsh-client-runtime、dsh-client-ui-slots、dsh-host-webserver 等）。
...
- **测试**：`node test\smoke.mjs`（124 项，用真实 hermes 文件副本验证；搜索测试自包含，不依赖真实文件内容——在线插件可能已合并改写）。测试需在 workspace 内建 junction `node_modules\@deepseek-ai` → 宿主 dsh 副本（`files` 白名单不含 node_modules，不影响安装）。含工具层测试（memory-tool execute 直连项目 store）、并发写测试与向量索引测试（假 embedding provider 验证增量同步/余弦/RRF）。
...
4. **验证**：`node test\smoke.mjs`（124 项，exit=0）+ `node --check` 三个入口文件 + 模块级端到端测试（备份/恢复/patch 写回用临时目录，**测试内严禁删真实数据目录**）。
...
- **改 `currentConfig` 极易踩「回显 ≠ 生效」**：它是 `lib/admin.js` 的**模块级导出纯函数 `currentConfig(cfg)`**（唯一调用点 `config: currentConfig(cfg)`）。实现是 **schema 驱动**：遍历 `CFG_SCHEMA`，对每个键取 `cfg[key] !== undefined ? cfg[key] : CFG_DEFAULTS[key]`，仍为 undefined 则回落 `''`，**`embeddingApiKey` 显式跳过**（密钥永不外发）。三条硬约束：①**除密钥外的每个 schema 键都必须以具体值出现在回显里**——客户端 `client/client.js:186-188` 只把「存在于 `s.config`」的键塞进表单状态 `cfgForm`，而渲染循环 `client/client.js:518-535` 遍历的是**整个 `state.schema`**、取值 `cfgForm[key]`，所以**键缺失不会让控件消失，而是渲染成一个空框**（`vectorIndexDir`/`embeddingBaseUrl` 曾长期显示为空，2026-09-12 在实机设置页实测确认）；②**已解析 cfg 必须压过默认值**，否则设置页对每个字段都显示 `CFG_DEFAULTS` 而运行时用的是真值——0.1.20 及以前就是这个状态（页面 5000 / 实际 8000）；③**回显错值不只是显示问题**：配置卡保存时把整个表单回贴（`client/client.js:255` `api('/configSave', cfgForm)`），错误回显会被**写回 profile patch**，等于把 5000 固化成真值。回归断言见 `test/smoke.mjs`（10 条），改动后必须跑。
- **判断限制值以运行时为准，别信页面**：`memory` 工具末行 `Usage: N% (chars / limit X)` 是运行时真值；`status` 的 `stores[].usagePct` 反推亦可交叉验证。
